...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/mattbrictson/mighty_test/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("mighty_test.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in mighty_test.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "mighty_test.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
